...
class simulateBacteria:
    ...
    def func(self, t, r):
        ...
        dx2_dt =  x1/(2 * 10**7) - extractedInsulin
...
def main():
    ...
    # This is without the extraction.
    bac2, insul2 = sim2.sol.y

    total_insulin_no_extraction = itg.trapz(sim2.sol.y[1], sim2.sol.t)

    print(total_insulin_no_extraction)


    print(sum(sim1.cumsumInsulin))


    plt.figure
    plt.plot(t_eval, bac, label = r'Bacteria')
    plt.plot(t_eval, insul, label = 'Insulin in the Vat')
    ...
    plt.title('Bacteria/Insulin Growth with Extraction: Rate =  0.3')
    plt.legend()
    plt.savefig('ExtractionPlot.png')

    plt.show()


    plt.figure
    plt.plot(t_eval, bac2, label = r'$Bacteria$')
    plt.plot(t_eval, insul2, label = r'$Insulin$')
    plt.xlabel('minutes')
    plt.ylabel('log(population density)')
    plt.yscale('log')
    #plt.title('Insulin extraction scheme')
    plt.title('Bacteria/Insulin Growth')
    plt.legend()
